...
in {
  ...
  config = let
    ...
    bspwmConfig =
      ...
      ''
      + (
        if cfg.polybar.enable
        then ''
          polybar-msg cmd quit
          ${
            if cfg.polybar.top.enable
            then "polybar --config=$XDG_CONFIG_HOME/polybar/config.ini top &"
            else ""
          }
          ${
            if cfg.polybar.bottom.enable
            then "polybar --config=$XDG_CONFIG_HOME/polybar/config.ini bottom &"
            else ""
          }
        ''
        else ""
