...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    print("Generating age key pair to decrypt sops secrets...")
    subprocess.run(["age-keygen", "-o", f"{tmpdir}/{host}.key"], check=True)
    age_pub = (
        subprocess.run(
            ["age-keygen", "-y", f"{tmpdir}/{host}.key"],
            check=True,
    ...
    subprocess.run(
        ["ssh-keygen", "-q", "-t", "ed25519", "-f", f"{tmpdir}/id_ed25519", "-C", ""],
        check=True,
    )
