...
if [ "$WITH_PASSWORD" -eq 1 ]; then
	echo "Locking key_vol with TPM2 and password."
else
	echo "Locking key_vol with TPM2 only."
fi
...
if [ "$WITH_PASSWORD" -eq 1 ]; then
	...
	exe systemd-cryptenroll /dev/zvol/zroot/key_vol --unlock-key-file=$KEY_VOL_KEY --tpm2-device=auto --tpm2-with-pin=no --tpm2-pcrs=7
fi

if [ "$WITH_SSH" -eq 1 ]; then
	echo "Locking rd_shared with TPM2 only."

	exe systemd-cryptenroll /dev/zvol/zroot/rd_shared_vol --wipe-slot=tpm2
	exe systemd-cryptenroll /dev/zvol/zroot/rd_shared_vol --unlock-key-file=$RD_SHARED_VOL_KEY --tpm2-device=auto --tpm2-with-pin=no --tpm2-pcrs=7
fi
